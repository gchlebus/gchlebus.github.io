...
def evaluate(refarr, testarr, th=0.1):
  '''
  Evaluate tumor segmentation contained in 'testarr' against reference 'refarr'. The function attemtps 
  to determine correspondences between test and reference tumor segmentation in order to handle correctly
  ...
  - determined tumor correpondences, e.g., [
    [[1, 2], [1]],
  ], meaning that reference tumors with indices 1 and 2 were used to determine whether the test tumor with 
  index 1 is a true positive. If the test passes, then the tumor would be counted as two true positives, 
  since it corresponds to two tumors in the reference.
...
def determine_correspondences(testidx, testarr, refarr):
  '''
  Determine tumor correspondences between tumors in the testarr and refarr. The correspondence algorithm 
  starts with a tumor with index textidx from the testarr.
  Returns:
  ...
  ref_components_count = len(unique(current_refarr))
  while True:  
...
def get_overlapping_mask(arr_a, arr_b, overlap=None):
  '''
  Returns all tumors from arr_b which overlap with the structure in arr_a according to the overlap function.
  The overlap function should receive two binary ndarrays and returns whether they overlap.
  The defualt overlap function tests for a non empty interesction.
  '''
...
def jaccard(refarr, testarr):
  assert not np.any(refarr > 1)
  assert not np.any(testarr > 1)
