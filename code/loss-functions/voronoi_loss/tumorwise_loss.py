...
if __name__ == '__main__':
  ...
  unique_ids, _ = tf.unique(tf.reshape(voronoi_var, [-1]))
  size = tf.size(unique_ids)
  op = tf.while_loop(cond, body, [i, size, dicesum, unique_ids, voronoi_var, labels_var, pred_var])
  with tf.Session() as s:
    s.run(tf.global_variables_initializer())
    print('First run')
    _, _, dicesum, *_ = s.run(op)
    print(dicesum)
    print('Second run')
    _, _, dicesum, *_ = s.run(op)
    print(dicesum)